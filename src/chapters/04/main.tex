...
% TODO Add: 'Therefore the overall cost, C, would be the cost if being unproductive, plus the cost of the medicine, M, that is \( C = K \int_{0}^{\infty}I(t)dt + M I(0) \).
...
% TODO Why do we set cost_of_cure = 5 here instead of just cost_with_cure = get_cost(I_0=I_0, alpha=2 * alpha, cost_of_cure=5) ?
...
% TODO Why do we set cost_of_cure <- 5 here instead of just cost_with_cure <- get_cost(alpha=2 * alpha, cost_of_cure = 5) ?
