...
The approaches are essentially equivalent, but they do require a fundamentally
different way of thinking about the system.
...
events occur that cause further events to occur in the future. These events can
either be unscheduled, that is they must occur immediately after their cause
event; or they can be scheduled events, meaning they are scheduled for a set
time in the future, usually after a delay from the cause event. Which events
need to be scheduled, and when they are to be scheduled, can be determined by a
number of factors, including the state of the system and the outcome of random
events.

In the bicycle shop, one scheduled event may be the arrival of a bicycle to the
shop (scheduled to occur some random amount of time after the previous arrival).
Depending on the state of the system, this could cause a number of other events
to occur:

\begin{itemize}
  \item if there are bicycles already waiting for the inspection counter, then
  the unscheduled event occurs in which the new arrival joins the queue;
  \item if the is no queue at the inspection counter, then the unscheduled event
  occurs where the new arrival begins service at the inspection counter, and
  another event is scheduled after some delay where that bicycle finished
  service.
\end{itemize}

The simulation progresses by jumping the clock forward to the next scheduled
event, carrying out all possible events that are to occur there (and any
unscheduled events that occur immediately after them), scheduling any other
events for the future, and then repeating this process by jumping the clock
forward to the next scheduled event in this newly updated list of events.

This is summarised in the diagram below:

\includestandalone[width=\textwidth]{./assets/event-scheduling}
