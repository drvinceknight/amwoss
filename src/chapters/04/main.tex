...
The first step is to define the symbolic variables that will be used.
The Python library SymPy is used which allows symbolic calculations.
...
def get_cost(
    ...
    return (
        sym.integrate(I_sol.rhs, (t, 0, sym.oo)) * cost_per_person
        + cost_of_cure * I_0
    )
\end{pyin}
...
\begin{Rin}
der <- derivative(t = 0, y = c(I = 100), parameters = c(alpha = 2))
print(der)
\end{Rin}
