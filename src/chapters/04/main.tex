...
mathematically using a \index{differential} equation which is a way of relating the rate
...
Finding a solution to this \index{differential} equation means finding an expression for
...
Now write a function to obtain the \index{differential} equation.
...
\index{SymPy} itself can be used to verify the result, by taking the derivative of the
...
the problem will be solved using a \index{numerical integration} approach. For an
...
of \index{differential} equations. Topics omitted here include the actual solution
...
In~\cite{syms2015dynamic} \index{differential} equations are used to build a
...
modelling \index{queueing networks}. This is interesting in its inception as
