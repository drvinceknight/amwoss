...
The differential function equation describing this will be of the form:
...
which individuals recover by will be denoted by \(\alpha\), then the
...
% TODO Is it worth 'showing' that this is indeed a solution, e.g. \(\frac{dI}{dt} = -\alpha e ^ {-\alpha y} = -\alpha I). It's something our first year students need to be shown.
...
Now function will be written to obtain the solution to this differential with
...
% TODO a very minor point: why is this not called `get_solution` like the Python code? For a casual reader, especially without the background given in the next chapter, the word 'integrate' might be confusing, because we're integrating again when caluclating the costs, which here we are just calling `area under the curve`.

% TODO is #noline functional? It's showing in the pdf.
...
Note that this function uses \mintinline{R}{stopifnot} to make sure the
differential equation has been solved for a long enough time period.
% TODO Is this needed? I dont use it in Chapter 5. If it is needed, this explanation isn't clear enough.
