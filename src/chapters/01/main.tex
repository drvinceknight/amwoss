...
\index{travelling salesman problem} or even if you get paid to do this stuff:
...
the so called ``worst plays in \index{Superbowl} history was''. Using an area of
...
The other author thinks the Superbowl is great, Geraint, used mathematics to find the best team of
\index{Pokémon}. Using an area of mathematics called \index{linear} programming (seen in
...
is also referred to as \index{Kerckhoffs}'s principle which states that ``a cryptosystem
...
\item
  The wider context. This section will include a few hand picked academic papers relevant to the
...
the Python library \index{Ciw}~\cite{ciwpython, palmer2019ciw}
is used whereas an alternative could be to use a Python
library called \index{SimPy}~\cite{simpy}.
...
\begin{itemize}
          \item \index{Modularity}: code is written as a
          ...
          \item \index{Documentation}: readable variable names as
              well as
              ...
              understandable.
          \item \index{Tests}: there are places where each module of code is
