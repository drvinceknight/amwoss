...
cost on the computer, similar to a video game.
...
Ciw uses the event scheduling approach, which means define the system's
facilities must be defined so that the customers may interact with them.
...
% TODO Discuss: do we need these smaller examples/explanations? (VK I think
% they are nice.)
...
the simulation, setting a seed ensures reproducible
results.
...
a more accurate proportion this experiment should be repeated, and an average
proportion taken.  In order to do so, the following is a function that performs
the above experiment over a number of trials, then finds an average proportion:
