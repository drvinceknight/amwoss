...
\index{probabilistic}. For example the
...
\index{discrete event simulation} (\index{DES}).
...
However due to a theorem called the \index{law of large numbers}~\cite{stewart2009probability},
...
They can however generate \index{pseudorandom} numbers: sequences of numbers that look
like random numbers, but are entirely determined from the
previous numbers in the sequence\footnote{An early
discussion of pseudorandom numbers is~\cite{von195113}. A number of different pseudo random
...
variables, each generated using \index{pseudorandom} numbers.
...
The process outlined above is a particular implementation of \index{Monte Carlo}
...
systems such as this one: \textit{\index{event scheduling}} and
\textit{\index{process based}} simulation.  It so happens that the
implementations in Python and R shown here use each of these approaches respectively.

% Event scheduling
\subsection{Event Scheduling Approach}

When using the \index{event scheduling} approach, the `virtual representation' of the
...
When using \index{process based simulation}, the `virtual
...
In this book the \index{Ciw}~\cite{ciwpython, palmer2019ciw}
...
In \index{Ciw}, these are defined as part of a Network object, created using the
...
Notice here a \index{random seed} is set. This is because there is randomness in running
...
than 0.5 hours the \index{Pandas}~\cite{reback2020pandas, mckinney-proc-scipy-2010}
...
However this particular day may have contained a number of \index{extreme
events}.
...
In this book the \index{Simmer}~\cite{ucar2019simmer} package will be used in order to conduct discrete event
...
Notice here a \index{random seed} is set. This is because there are elements of
...
creating valid models of real life scenarios. These include \index{time
dependent rates}
and \index{rostering servers}. An overview of the theory of discrete event simulation is
given in~\cite{robinson2004simulation}.

One particular use of discrete event simulation is as part of a wider
\index{optimisation} exercise. For example, a systematic search for an optimal service
...
One domain where simulation is often used is in modelling \index{healthcare} systems.
...
methodology with \index{systems dynamics} (Chapter~\ref{chp:system_dynamics}) in order
to model continuous aspects of the system and/or \index{agent based modelling}
(Chapter~\ref{chp:agent_based_simulation}) in order to observe emergent or
learned behaviours. This is known as \index{hybrid simulation}, and an overview is given
