...
\index{susceptible} to infection by some disease. This population can be described by
...
\begin{itemize}
  ...
  day;
  \item \index{infectious} people recover naturally and gain an immunity from the
...
$\pounds 220$ per vaccine. The \index{healthcare} providers would like to know if
achieving a \index{vaccination rate} $v$ of $85\%$ would be beneficial financially.

\section{Theory}\label{sec:system_dynamics_theory}

The above scenario can be expressed using a compartmental model of disease, and can be
...
($R$)\footnote{This is often referred to as an \index{SIR} model.}. The
levels on these \index{stocks} change according to the flows in, out, and between them,
controlled by `taps'. The amount of flow the \index{taps} let through are influenced in
...
Mathematically the quantities or \index{stocks} are functions over time, and the change
in stock levels are written as the \index{derivatives}, for example the change in the
...
described by the following system of \index{differential} equations:
...
under these rules can be quantified by solving this system of
\index{differential}
...
methods for this kind of problems use the principle that the \index{derivative} denotes
the rate of instantaneous change. Thus for a \index{differential} equation
...
Thus numerical methods, including the \index{Runge-Kutta} methods and the
\index{Euler}
method\footnote{
...
Here the \mintinline{python}{solve_ivp} method of the \index{SciPy}~\cite{2020SciPy-NMeth}
library will be used to numerically solve the above models.

First the system of \index{differential} equations described in Equations~\ref{eqn:dS},
...
this means that the number of \index{susceptible} individuals is expected to reduce by
around 0.255 per time unit, the number of \index{infected} individuals to increase by
0.21 per time unit, and the number of \index{recovered} individuals to increase by 0.045
...
using \index{SciPy}'s \mintinline{python}{solve_ivp} to numerically solve the system
of \index{differential} equations:
...
\index{vaccination} rate of 0\% and a \index{vaccination} rate of 85\%. The system will now be
observed for two years, that is 730 days.

Begin with a \index{vaccination} rate of 0\%:
...
The number of \index{infected} individuals increases quickly, and in fact the rate of
change increases as more individuals are infected. However this growth slows
down as there are fewer \index{susceptible} individuals to infect. Due to the equal
birth and death rates the overall population size remains constant; but after
some time period (around 200 time units) the levels of \index{susceptible},
\index{infected},
and \index{recovered} individuals stabilise, and the disease becomes
\index{endemic}. Once this
...
With vaccination the disease \index{remains} endemic, however once steadiness occurs,
around 10\% of the population remain \index{susceptible} to the disease, 1.7\% are
infected, and 88.3\% are immune or recovered and immune.

This shows that \index{vaccination} lowers the percentage of the population living with
the infection, which will lower the public \index{healthcare} system's costs. This
saving will now be compared to the cost of providing the \index{vaccination} to the
newborns.

The following function calculates the total cost to the public
\index{healthcare}
...
Now the total daily cost with and without \index{vaccination} can be compared.
Without \index{vaccinations}:
...
Therefore without \index{vaccinations}, once the \index{infection} is
\index{endemic}, the public health
care system would expect to spend $\pounds 9002.33$ a day.

With a \index{vaccination} rate of 85\%:
...
So \index{vaccinating} 85\% of the population would cost the public health care system,
...
The \index{deSolve}~\cite{soetaert2010solving} library will be used to numerically
solve the above \index{epidemiology} models.
...
The number of \index{susceptible} individuals is expected to reduce by around 0.255 per
time unit, the number of \index{infected} individuals to increase by 0.21 per time unit,
and the number of \index{recovered} individuals to increase by 0.045 per time unit.
After a tiny fraction of a time unit these quantities will change, and thus the
rates of change will change.

The following function observes the system's behaviour over some time period,
using the \index{deSolve} library to numerically solve the system of
...
a \index{vaccination} rate of 0\% and a vaccination rate of 85\%. The system will be
...
using Python and the \index{Matplotlib} library~\cite{hunter2007matplotlib}. R has powerful
plotting tools with packages such as \index{ggplot2}~\cite{wickham2016ggplot2}.}
...
some time period (around 200 time units) the levels of \index{susceptible},
\index{infected},
and \index{recovered} individuals stabilises, and the disease becomes \index{endemic}. Once this
...
With \index{vaccination} the disease remains endemic, however once steadiness occurs,
around 10\% of the population remain \index{susceptible} to the disease, 1.7\% are
infected, and 88.3\% are immune or recovered and immune.
...
system, that is the sum of the medication costs for those living with the
infection and the \index{vaccination} costs:
...
The total daily cost with and without \index{vaccination} will now be compared. Without
vaccinations:
...
Therefore without vaccinations, once the infection is \index{endemic}, the public health
...
So vaccinating 85\% of newborns would cost the public \index{healthcare} system, once
...
\index{dynamical systems}. \cite{robinson1998dynamical} gives a mathematical
...
In the field of operational research, \index{Jay Forrester} is recognised as the first
person to use dynamical systems in the way shown in this chapter.
His own account can be read
...
of \index{healthcare}. For example, \cite{cooper2020sir} applies the same model
from this chapter to the study of the \index{COVID} pandemic.
...
with \index{discrete event simulation} (see Chapter~\ref{chp:discrete_event_simulation})
in order to model discrete aspects of the system
and/or \index{agent based} modelling (see Chapter~\ref{chp:agent_based_simulation}) in
order to observe emergent or learned
behaviours. This is known as \index{hybrid} simulation, and an overview is given
