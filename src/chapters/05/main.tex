...
susceptible to infection by some illness. This population can be described by
...
\begin{itemize}
  ...
  illness), at a recovery rate $r$ of 0.02 per day;
...
The above scenario is called a compartmental model of illness, and can be
represented
in a stock and flow diagram as in Figure~\ref{fig:stockflow}.
...
susceptible to illness ($S$), those infected with the illness ($I$), and those
who have recovered from the illness and so have gained immunity ($R$). The
...
% TODO Add footnote reference text.
...
this means thati the number of susceptible individuals is expected to reduce by
around 0.255
...
unit. After a tiny fraction of a time unit the stock levels will
...
a vaccination rate of 0\% and a vaccination rate of 85\%. The system will now be
observed
for two years, that is 730 days, in time steps of 0.01 days.
...
The number of infected individuals increases quickly, and in
...
but after some time period (around 300 time units) the levels of
susceptible, infected, and recovered individuals stabilise, and
the illness becomes endemic. Once this occurs, around
10\% of the population remain susceptible to the illness, 30\% are infected, and
...
With vaccination the illness remains endemic, however once
steadiness occurs, around 10\% of the population remain susceptible to the
illness, 1.7\% are infected, and 88.3\% are immune or recovered and immune.
...
This saving will now be compared to the cost of providing the
...
The \mintinline{R}{deSolve} library will be used to numerically
solve the above epidemiology models.

First the system of differential equations described in Equations~\ref{eqn:dS},
\ref{eqn:dI} and \ref{eqn:dR} must be defined. This is done using an
R function, where the arguments are the current time, system state and a list of
other parameters.
...
three stocks, $S$, $I$ and $R$. Starting at time 0.0, with 4 susceptible
...
The number of susceptible individuals is expected to reduce by around 0.255
per time unit, the number of infected individuals to increase by 0.21 per time
unit, and the number of recovered individuals to increase by 0.045 per time
unit. After a tiny fraction of a time unit the stock levels will
change, and thus the rates of change will change.
...
the illness becomes endemic. Once this steadiness occurs, around
10\% of the population remain susceptible to the illness, 30\% are infected, and
60\% are recovered and immune.
