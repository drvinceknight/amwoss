...
% TODO Discuss this section: the Rsympy package looks un cared for and in fact
% seems to have dependency issues because of the way it uses java. All of the
% following code is **NOT TESTED** because I cannot test it without getting a
% weird IO error from java.
R does not have a bespoke symbolic mathematics library however it has an
interface to use Python's \mintinline{python}{sympy} called
\mintinline{R}{rSymPy}. In general we would recommend to use Python directly
when needing to carry out symbolic mathematical calculations however this is not
always possible. Furthermore, the development of \mintinline{R}{rSymPy} seems to
no longer be active and due to the dependency on Java could be problematic to
use.
One of other the difficulties of using \mintinline{R}{rSymPy}
is that it works by passing string back and forth to Python's
\mintinline{python}{sympy}. As such, we will need to have more of a manual
