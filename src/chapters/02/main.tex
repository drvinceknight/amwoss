...
A Markov chain is a model of a sequence of \index{random events} that is defined by a
...
10 per unit time. The state decrease when people are served and this happens at a rate of 4
per active server per unit time. In both cases it is assumed that no 2 events can occur at
the same time.

In general, the rules that govern how to move between these states can be defined in 2
ways:

\begin{itemize}
    \item Using probabilities of changing state (or not) in a well defined time
        interval. This is called a \index{discrete time Markov chain}.
...
Note that a Markov chain assumes the rates follow an \index{exponential
distribution}.
...
These states and rates can be represented mathematically using a \index{transition
...
gives a description of how to compute the \index{matrix exponential}
numerically and~\cite{moler1978nineteen,
...
This \index{long run probability distribution} over the states can be represented using a
\index{vector} \(\pi\) where \(\pi_i\) represents the probability of being in state
\(i\). This vector is in fact the solution to the following \index{matrix} equation:
...
with the following \index{constraint}:
...
problem is written. The \index{Numpy}~\cite{harris2020array} library will be used to
handle all the \index{linear algebra} and the \index{Itertools} library for
some iterations:
...
discussed above, using the \index{SciPy}~\cite{2020SciPy-NMeth}
...
can be solved directly in order to get a solution where \index{equilibrium} is
guaranteed.

The underlying linear system will be solved using a \index{numerically} efficient
algorithm called \index{least squares} optimisation (available from the
Numpy library):
...
version\footnote{
A \index{vectorized} calculation refers to the manner in which an instruction is given
...
\index{expm}~\cite{goulet2021expm} will be used.
To be able to make use of the nice \mintinline{R}{%>%} ``pipe'' operator the
\index{magrittr}~\cite{bache2020magrittr} library will be loaded. Now to see
what would happen after 0.5 time units:
...
To be able to do this, the versatile \index{pracma}~\cite{borchers2021pracma} package
...
of \index{steady state} probabilities. There are in fact many more theoretic and applied
aspects of Markov chain models. Some examples of this include the calculation of
\index{sojourn} times which is how long a system spends in a given state as well as
considering models with \index{absorption}: where the system arrives at a state that it
...
\index{board games}. In~\cite{tan1997markov} a model of the battles that take place on
...
queueing systems applied to \index{healthcare}. In~\cite{griffiths2013modelling} a
...
An extension of Markov chains are \index{Markov decision processes}. This is a
