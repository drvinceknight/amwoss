...
happen and potentially give a financial \index{incentive} to each company to ensure
their \index{behaviour} is in the population's interest.
...
For example, from Table~\ref{tbl:profit-of-taxi-companies-offset} it can be seen that if Company B
...
object called a \index{game}, which here requires:

\begin{enumerate}
    \item a given collection of actors that make decisions (\index{players});
    \item options available to each player (\index{actions});
    \item a numerical value associated to each player for every possible
        choice of action made by all the players. This is the \index{utility} or reward.
\end{enumerate}

This is called a \index{normal form game} and is formally defined by:
...
When \(N=2\) the \index{utility} function is often represented by a pair of
\index{matrices} (1
...
A \index{strategy} corresponds to a way of choosing actions, this is represented by a
...
correspond to both taxi firms choosing the \index{strategy}: \((0, 1, 0)\). 
...
This is called a \index{Nash equilibrium}: when both players are playing a
strategy that is the \index{best response} against the other.

An important fact is that a \index{Nash equilibrium} is guaranteed to exist.
This was actually the theoretic result for which \index{John Nash} received a Nobel
Prize\footnote{
\index{John Nash} proved the fact that any game has a Nash equilibrium
...
There are various algorithms that can be used for finding \index{Nash
...
in \index{linear programming} (see Chapter~\ref{chp:linear_programming}).
One such example is called the \index{Lemke-Howson} algorithm.
...
equilibrium, such emergent processes are the topics of \index{evolutionary} game
theory\footnote{
Evolutionary game theory was formalised in~\cite{smith1974theory} although
some of the work of Robert \index{Axelrod} is \index{evolutionary} in
principle~\cite{axelrod1990evolution}.
},
\index{learning algorithms}\footnote{
An excellent text on learning in games is~\cite{fudenberg1998theory}.
}
and/or \index{agent based} modelling which will be covered in
...
matrix of expected profits and any offset. The \index{Nashpy}~\cite{knight2018nashpy}
and \index{Numpy}~\cite{harris2020array} libraries will be used for this.
...
will choose to use \index{Recon}~\cite{oliveira2019recon} which has functionality for finding the
...
example \index{extensive form games} which are represented by trees and more explicitly model
...
process: \index{Moran processes} or \index{replicator dynamics}. A good text book to read on
these topics is~\cite{maschler2013game}.

\index{John Nash} whose life was portrayed in the 2001 movie ``A Beautiful Mind'' (which
...
behaviour. \index{Axelrod}~\cite{axelrod1990evolution} started this work with
computer tournaments where he invited code submissions of strategies to play a
game called the \index{Iterated Prisoner's Dilemma}. The outcome of this was an
...
more recent work involving so called \index{Zero-Determinant}
