...
\index{agent based} simulation, and is referred to as \index{Schelling}'s segregation model.
It features in Thomas \index{Schelling}'s book `Micromotives and
Macrobehaviours'~\cite{schelling2006micromotives}, whose
title neatly summarises the world view of \index{agent based} modelling: we know,
...
In general an agent based model consists of two components, \index{agents}, and an
\index{environment}:

\begin{itemize}
    \item \index{agents} are autonomous entities that will periodically choose to take one
        of a number of actions.
        These are chosen in order to maximise that agent's own given utility
        function;
    \item an \index{environment} contains a number of agents and defines how their
...
attributes from some \index{learning process}; with the ultimate aim of
maximising its own utility. In practice, a \index{utility} can be represented by a
function that maps the environment to some numeric value.
This process happens to all \index{agents} in the \index{environment}, possibly simultaneously.
...
For the football team supporters problem, each \index{household} is an agent.
The environment is the city.
Each household's \index{utility} function is to satisfy their preference of living next
...
\index{object-orientated programming}.
...
\textit{\index{attributes}}), and do things (in Python these are called
\textit{\index{methods}}).
\index{Object-orientated programming} allow for the creation of new
\index{classes} which can be
used to implement the individual behaviours of an \index{agent based} model.
...
method, which is only used to observe any \index{emergent behaviour}.
...
This was the central premise of \index{Schelling}'s original
model~\cite{schelling2006micromotives}
that overall
emergent \index{segregation} levels are much higher than any individuals' personal
preference for \index{segregation}.
...
\textit{\index{fields}}), and do things (in the R package used here these are called
\textit{\index{methods}}).
\index{Object-orientated programming} allow for the creation of new \index{classes} which can be
...
In this chapter, a package called \index{R6} will be used here.
...
method, which is used to observe the \index{emergent behaviour}.
...
The model described here is called \index{Schelling}'s segregation model, and was
...
in~\cite{reynolds1987flocks}, otherwise referred to as \index{Boids}, where the
behaviours of flocks of birds are understood by capturing the individual
interactions between individual birds. Conway's \index{Game of Life}, described
in~\cite{gardener1970mathematical} is another classic, which comes under
the banner of cellular \index{autonoma}. Here cells on a grid either become alive or dead
...
\index{Axelrod}~\cite{axelrod1990evolution},
