...
% These paragraphs are not relevant to the problem, they comment on the solution method. I have moved them and reworded them slightly into the next section.
...
This is an example of what is known as the Travelling Salesman Problem, %TODO add footnote reference
...
example, the tour \(t^{(1)}\in N(t)\) is obtained by swapping the 2rd and 5th
stops.
...
\begin{figure}[!hbtp]
    ...
    \end{center}
    \caption{The effect of two neighbourhood operators on \(t\). \(t^{(1)}\) is
...
% TODO Why give the option to not pass a seed? This adds extra lines/complexity to the code that is not used.
...
def run_neighbourhood_search(
    ...
    return candidate
\end{pyin}
% TODO `number_of_stops` is just len(distance_matrix).
...
an improvement on the solution found using the \mintinline{python}{swap_stops}
...
This gives a random tour on 13 stops:
% TODO Same as with Python, Why give the option to not pass a seed? This adds extra lines/complexity to the code that is not used.
...
To be able to evaluate any given tour its cost must be found. Here
\mintinline{R}{get_cost} to do this:
...
Now a function for neighbourhood operator will now be written,
\mintinline{R}{swap_stops}, that swaps two stops in a given tour.
...
an improvement on the solution found using the \mintinline{R}{swap_stops}
